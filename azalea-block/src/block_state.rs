...
mod tests {
    ...
    fn test_from_blockstate() {
        ...
        let block: Box<dyn BlockTrait> =
            Box::<dyn BlockTrait>::from(BlockState::from(Block::FloweringAzalea));
        assert_eq!(block.id(), "flowering_azalea");
    }

    #[test]
    fn test_debug_blockstate() {
        let formatted = format!("{:?}", BlockState::from(Block::FloweringAzalea));
        assert!(formatted.ends_with(", FloweringAzalea)"), "{}", formatted);

        let formatted = format!("{:?}", BlockState::from(Block::BigDripleafStem));
        assert!(
