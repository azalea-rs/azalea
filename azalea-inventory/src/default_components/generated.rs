...
impl DefaultableComponent for BlockState {
    fn default_for_item(item: Item) -> Option<Self> {
        let value = match item {
            Item::BeeNest => {
                HashMap::from_iter([("honey_level".to_string(), "0".to_string())].into_iter())
            }
            Item::Beehive => {
                HashMap::from_iter([("honey_level".to_string(), "0".to_string())].into_iter())
            }
            Item::Light => {
                HashMap::from_iter([("level".to_string(), "15".to_string())].into_iter())
            }
            Item::TestBlock => {
                HashMap::from_iter([("mode".to_string(), "start".to_string())].into_iter())
            }
            _ => return None,
        };
...
impl DefaultableComponent for StoredEnchantments {
    fn default_for_item(item: Item) -> Option<Self> {
        let value = match item {
            Item::EnchantedBook => HashMap::from_iter([].into_iter()),
            _ => return None,
        };
