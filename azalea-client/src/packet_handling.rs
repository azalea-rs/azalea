                ...
                match &p.action {
                    Action::AddPlayer(v) => {
                        for new in v {
                            ...
                            tab_list.insert(new.uuid, info.clone());
                    ...
                    Action::UpdateGameMode(v) => {
                        for update in v {
                            if let Some(info) = tab_list.get_mut(&update.uuid) {
                                ...
                                warn!("Ignoring UpdateGameMode for unknown player {}", update.uuid);
                            }
                        }
                    }
                    Action::UpdateLatency(v) => {
                        for update in v {
                            if let Some(info) = tab_list.get_mut(&update.uuid) {
                                ...
                                warn!("Ignoring UpdateLatency for unknown player {}", update.uuid);
                            }
                        }
                    }
                    ...
                    Action::RemovePlayer(v) => {
                        for update in v {
                            if let Some(info) = tab_list.remove(&update.uuid) {
                                remove_player_events.send(RemovePlayerEvent {
                                    entity: player_entity,
                                    info,
                                });
                            } else {
                                warn!("Ignoring RemovePlayer for unknown player {}", update.uuid);
                            }
                        }
                    }
                }
